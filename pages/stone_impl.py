...
def get_attachments(order_id):
    return read("SELECT id,file_name,mime_type,file_size,file_data,created_at FROM order_attachments WHERE order_id=? ORDER BY id", (int(order_id),))
...
st.title("석재 관리")
st.caption("타일과 같은 방식으로 관리 · 석재구분: 인조석 / 천연가공석")
st.caption("☁ 중앙 DB 연결")

# 1) 관리자 예산/품목 관리: 입력 중 저장되지 않고 저장 버튼에서만 DB 반영
if is_admin():
    st.markdown("### 관리자 석재 예산 / 품목 관리")
    st.info("예산수량·규격·단위·석재구분을 한 번에 수정하고 저장합니다.")
    ...
    with st.form("stone_admin_budget_form"):
        edited = st.data_editor(
            edit, use_container_width=True, hide_index=True, disabled=["id"],
            column_config={
                "id": None,
    ...
    if save_budget:
        for _, r in edited.iterrows():
            execute("""UPDATE budget_items SET vendor=?,tile_type=?,item_name=?,spec=?,unit=?,budget_qty=?,default_destination=? WHERE id=? AND category=?""", (
                str(r["협력사"] or "").strip(), str(r["석재구분"] or "").strip(), str(r["품명"] or "").strip(),
                str(r["규격"] or "").strip(), str(r["단위"] or "").strip(), float(r["예산수량"] or 0),
                str(r["기본납품처"] or "").strip(), int(r["id"]), CATEGORY))
    ...
    with st.form("stone_admin_new_item_form", clear_on_submit=True):
        ...
        a1,a2 = st.columns(2)
        new_type = a1.selectbox("석재구분", STONE_TYPES)
        new_vendor = a2.text_input("협력사")
        a3,a4 = st.columns(2)
        new_name = a3.text_input("품명")
        new_spec = a4.text_input("규격")
        a5,a6,a7 = st.columns(3)
        new_unit = a5.text_input("단위", value="M")
        new_qty = a6.number_input("예산수량", min_value=0.0, value=0.0, step=0.1)
        new_dest = a7.text_input("기본납품처", value="현장")
    ...
    if add_new:
        if not new_name.strip() or not new_unit.strip():
            ...
            if len(dup):
                ...
                execute("""INSERT INTO budget_items(category,vendor,item_name,spec,unit,budget_qty,tile_type,application_type,default_destination,active) VALUES(?,?,?,?,?,?,?,?,?,1)""", (
...
# 협력사 품목 등록 화면 제거: 관리자가 예산/품목 관리에서 신규 등록

# 2) 투입내역: 모든 입력을 하나의 form으로 묶어 입력 중 rerun/DB 저장 방지
if len(df):
    st.markdown("### 투입내역 입력")
...
# 3) 발주서: 품목/납품정보/비고/첨부 모두 하나의 form에서 입력 후 마지막 저장만 실행
st.markdown("---")
st.markdown("### 석재 발주서 작성")
st.info("모든 내용을 먼저 입력한 뒤 맨 아래 저장 버튼을 눌러주세요. 입력 중에는 저장되지 않습니다.")
...
if vendors:
    with st.form("stone_order_form"):
        ...
        st.markdown("#### 품목 선택")
        req_edit = st.data_editor(req, use_container_width=True, hide_index=True,
        ...
        attachments = st.file_uploader("도해도 및 첨부파일 선택", accept_multiple_files=True)
    ...
    if save_order:
        ...
        if not len(selected):
            ...
            for _, r in selected.iterrows():
                ...
                execute("""INSERT INTO order_lines(order_id,item_id,qty,requested_delivery_date,destination,delivery_recipient,delivery_phone,delivery_address) VALUES(?,?,?,?,?,?,?,?)""",
                    (oid,item_id,qty,delivery_date,delivery_type,delivery_recipient.strip(),delivery_phone.strip(),delivery_address.strip()))
                execute("""INSERT INTO transactions(tx_date,item_id,tx_type,qty,destination,note,input_user) VALUES(?,?,?,?,?,?,?)""",
                    (str(order_date),item_id,"발주",qty,delivery_type,f"발주서 {order_no}",vendor))
    ...
    st.info("석재 품목이 등록되어야 발주서를 작성할 수 있습니다. 관리자 설정에서 신규 품목을 추가해주세요.")

if st.session_state.get("stone_last_pdf"):
    st.download_button("📄 석재 발주서 PDF 다운로드", st.session_state["stone_last_pdf"], file_name=st.session_state["stone_last_pdf_name"], mime="application/pdf", type="primary", key="stone_pdf_download")
            ...
            if len(at):
                ...
                for _, a in at.iterrows():
                    ...
                    if hasattr(raw, "tobytes"):
                        ...
                        raw = bytes(raw)
                    st.download_button(f"📎 {a['file_name']}", data=raw, file_name=a["file_name"], mime=str(a["mime_type"] or "application/octet-stream"), key=f"stone_attach_{int(a['id'])}")
            else:
