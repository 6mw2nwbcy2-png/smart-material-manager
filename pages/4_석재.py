...
def init_stone_db():
    # 기존 order_lines에 납품정보 컬럼이 없을 수 있는 초기 DB도 대응
    delivery_columns = [
...
def make_stone_pdf(order_row, lines):
    ...
    normal = ParagraphStyle("stoneNormal", parent=styles["Normal"], fontName=font, fontSize=8.5, leading=11, alignment=TA_CENTER)
    title = ParagraphStyle("stoneTitle", parent=styles["Title"], fontName=font, fontSize=19, leading=23, alignment=TA_CENTER, textColor=colors.HexColor("#153A5B"))
...
if len(df):
    ...
    st.info("등록된 석재 품목이 없습니다. 관리자 입력에서 인조석/천연가공석 품목을 등록하세요.")
...
if is_admin():
    ...
    st.caption("석재구분은 인조석 또는 천연가공석으로 선택합니다.")
...
if not len(orders):
    ...
    for _, order in orders.iterrows():
        with st.expander(f"{order['order_no']} | {order['vendor']} | {order['order_date']}"):
            ...
            if len(files):
                ...
                for _, f in files.iterrows():
                    st.download_button(
                        ...
                        mime=str(f["mime_type"] or "application/octet-stream"),
                        key=f"stone_attachment_{int(f['id'])}",
                    )
            else:
                st.caption("첨부파일 없음")
            ...
            st.download_button(
                "📄 이 발주서 PDF",
                make_stone_pdf(order, pdf_lines),
                file_name=f"{order['order_no']}_석재발주서.pdf",
                mime="application/pdf",
